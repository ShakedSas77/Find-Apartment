...
# Health check — verifies every dependency (env, credentials.json, Sheets, Gemini,
# Ollama, Telegram if enabled) without spending Google Maps/Distance Matrix quota
python doctor.py
python doctor.py --alert   # also DMs a Telegram alert if anything FAILs

# Timestamped SQLite backup of bot_data.db (the sole dedupe/verdict store) — schedule
# it yourself (e.g. weekly) if you want it automatic; no Task Scheduler wiring exists
python backup_db.py

# Telegram vote listener (only relevant if TELEGRAM_ENABLED=True) — separate
# long-running process, not part of the scrape
...
| File | Purpose |
|---|---|
| `apartment_bot.py` | CLI entrypoint only: `--stats`/`--reparse-rejected`/`--replay`/`--prune`/`--recompute-scores`/`--backfill-ratings`/`__main__` dispatch to `run_scraper()`. As of the Phase 0-9 extraction (2026-07-26), the scraping/LLM/Sheets logic that used to all live here has moved to the modules below — see each for what it owns. |
| `fb_scraper.py` | Playwright/FB-DOM-specific: checkpoint/CAPTCHA handling, dead-link pruning, post-URL canonicalization, article selectors, "See more" expansion, `_scan_group_page`/`_scan_group`, `run_scraper()` |
| `core/evaluate.py` | Source-agnostic listing pipeline: `process_candidate_listing()` (LLM parse → `_evaluate_post_data` → `_build_row` → queue), `_replay_text_prefilters()` |
| `core/llm.py` | Gemini/Ollama dual-path orchestration: `analyze_post_with_llm()`, `ApartmentData` schema, `get_gemini_client()`, `GEMINI_EXHAUSTED` |
| `maps.py` | Distance/geocoding: `get_walking_distance()`, `_validate_address_with_geocoding()`, `_format_address_display()`, `get_gmaps_client()` |
| `sheets.py` | Google Sheets I/O: `setup_google_sheet()`, `dedupe_and_sort_sheet()`, row-rewrite helpers |
| `core/normalize.py` | Pure text/domain normalization: dates, floor, fees, parking, address formatting, `BIDI_RE`, `_sheet_safe_cell` |
| `core/dedupe.py` | Cross-post dedup: `_text_dedup_hash()`, `_listing_dedupe_key()` |
| `browser.py` | Stealth patching, Chrome launch args, `_is_visible()`, `_dismiss_popups()` — shared by `fb_scraper.py` and `yad2_scraper.py` |
| `core/util.py` / `core/errors.py` | Shared low-level helpers (`_safe_print`, `_with_retries`, locks) and signal exceptions (`GmapsQuotaHalted`, `HeadlessCheckpointAbort`) — dependency-free, importable from anywhere without a cycle |
| `env.py` | `.env` access with no import-time validation (`require_env()` is called explicitly from `apartment_bot.py`'s `__main__`, not at import) — this is what makes every other module importable/testable without credentials |
...
| `scoring.py` | `compute_fit_score(fields)` — 0-100 fit score (price/distance/amenities) appended as the sheet's last column |
| `distance_fallback.py` | Free Nominatim geocode + calibrated haversine estimate, used only when `GMAPS_MONTHLY_CAP` is hit |
| `telegram_notifier.py` | Optional (`TELEGRAM_ENABLED`) — pushes new matches to a shared Telegram chat with ⭐/🗑 vote buttons |
| `bot_listener.py` | Separate long-running process — long-polls Telegram for vote button taps, records them, updates the tally shown on the message |
| `doctor.py` | Dependency health check (env, credentials.json, Sheets, Gemini, Ollama, Telegram) — run manually or `--alert` to DM on failure. Never calls Geocoding/Distance Matrix (shares `GMAPS_MONTHLY_CAP`'s counter) |
| `backup_db.py` | Timestamped `bot_data.db` snapshot to `backups/` (gitignored), keeps newest 14 |
...
| `.env` | `GEMINI_API_KEY`, `GMAPS_API_KEY`, `SHEET_ID` (required); `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` (optional, only if `TELEGRAM_ENABLED=True`) — all gitignored |
...
- Shared state guarded by module-level locks, now spread across the modules that own each piece of state: `_sheet_lock`/`_print_lock` (`core/util.py` — `_sheet_lock` covers both `seen_urls` reads/writes and `sheet.append_row`, kept together to avoid duplicate-URL races; `_print_lock` via `_safe_print` prevents interleaved terminal output), `_gemini_lock` (`core/llm.py`, guards `GEMINI_EXHAUSTED`/`GEMINI_ERROR_COUNT`), `_gmaps_cap_lock` (`maps.py`, guards the one-time over-cap notice print), `_text_dedup_lock`/`_headless_checkpoint_hit` (`fb_scraper.py`). `storage.py` has its own internal lock for all SQLite access (both `posts` and `api_usage`), so the Maps-usage counter is safe to increment from every group thread.
...
- **Global flag**: `GEMINI_EXHAUSTED` (module-level bool in `core/llm.py`)
...
- `SHEET_ID` lives in `.env` (not `config.py` — repo is public), accessed via `env.get_sheet_id()` alongside `env.get_gemini_api_key()`/`env.get_gmaps_api_key()`; `env.require_env()` (called from `apartment_bot.py`'s `__main__`, not at import) exits at startup if any of the three are missing. Sheet must be shared with the service account email from `credentials.json`
- 16 Hebrew headers defined in `config.py` (`SHEET_HEADERS`) — the 14 data columns, a `זמן סריקה` (scan timestamp) column, then a trailing `ציון התאמה` (fit score, see Fit Score section below)
...
- **Formula-injection guard** (`_sheet_safe_cell()`, security audit 2026-07-18): rows are written with `value_input_option="USER_ENTERED"` (needed for price/rooms to land as real numbers, not text), which also lets Sheets read a cell starting with `=`/`+`/`-`/`@` as a formula. Row content ultimately comes from scraped Facebook text via an LLM, so `_build_row()` (`core/evaluate.py`) runs every field through `_sheet_safe_cell()` (`core/normalize.py`), which prefixes such a string with a literal `'`. This is deliberate defense-in-depth — every current field already happens to be safe by construction (fixed value sets, numeric, or Hebrew-only text that excludes those characters) but that was incidental, not designed, before this guard existed.
...
- `api_usage` table (`month`, `gmaps_calls`) backs the Google Maps quota guard in `get_walking_distance()` (`maps.py`): before every `distance_matrix` call it checks `storage.get_gmaps_usage()` against `GMAPS_MONTHLY_CAP` (config.py, default 9000) and increments via `storage.increment_gmaps_usage()` right before the call (1 element per call). **The same counter also gates `_validate_address_with_geocoding()`'s Geocoding API call** (`maps.py`) — once the cap is hit, geocoding is skipped too, not just Distance Matrix. Over cap: `GMAPS_ON_CAP = "skip"` (default) tries `distance_fallback.estimate_straight_line_distance()` — a free Nominatim geocode + calibrated haversine estimate to `DESTINATION_LAT`/`DESTINATION_LON` (config.py) — before falling back to the placeholder string; `"halt"` raises `GmapsQuotaHalted`, caught in `_scan_group` (`fb_scraper.py`) to stop that group's remaining posts cleanly (other groups stop independently on their next distance check, since the counter is shared). A straight-line result gets `distance_source="straight_line_estimate"`, included alongside `"google_maps"` in `_DISTANCE_FILTERABLE_SOURCES` (`core/evaluate.py`) so the `> MAX_WALKING_DISTANCE_KM` rejection still applies to it — update that set, not just the `== "google_maps"` checks, if a third real distance source is ever added. Quota status prints once at the start of `run_scraper()`, plus a warning at ≥80% of cap.
- **`_redact_api_key()`** (`maps.py`, security audit 2026-07-18): `googlemaps` sends `GMAPS_API_KEY` as a URL query param (Gemini instead uses a header, so it's unaffected) — on a network-level failure the underlying `requests`/urllib3 exception's `str()` embeds the full request URL, key included. Both `except` blocks around `get_gmaps_client()` calls (`_validate_address_with_geocoding`, `get_walking_distance`) run the exception through `_redact_api_key()` before printing, so a transient network error can't leak the live key into console output or a log file.
- `_scan_group_page` (`fb_scraper.py`) calls `storage.should_skip(url)` immediately after `extract_post_info`, before the seen-in-sheet check: any cached verdict except `parse_failed` skips the post with zero LLM calls. `parse_failed` retries automatically, capped at `storage.MAX_PARSE_ATTEMPTS` (3).
- `_evaluate_post_data()` (rooms/price threshold checks + regex second-chance + field normalization) and `_build_row()` (sheet row assembly, calls `get_walking_distance`) — both in `core/evaluate.py` — are shared between `_scan_group_page` (`fb_scraper.py`) and `reparse_rejected_posts()` (`apartment_bot.py`) — keep both call sites in sync if this logic changes.
...
- CLI flags: `--stats` prints verdict counts + this month's Maps usage, then a per-group yield breakdown (flags any group with zero `added` as a candidate to drop from `TARGET_URLS`) and top `excluded_location`/`rejected_distance` values (via `storage.get_group_stats()`/`top_excluded_locations()`/`top_rejected_distance_addresses()`) to help tune `EXCLUDED_LOCATIONS`/`MAX_WALKING_DISTANCE_KM`, then exits; `--reparse-rejected` re-runs the LLM + filters against stored raw text for `rejected_price`/`rejected_rooms`/`rejected_distance`/`parse_failed`/`price_unknown` posts with **no Playwright/browser involved** (still uses the real Sheets API client, so genuine matches found on reparse are appended for real — always live, no dry-run gate on this flag; skips rows whose `raw_text` was already pruned, see below); `--replay` is the "I changed the filters/prompt/normalization, test it without re-scraping" workflow — **read-only**: it re-runs **every** post ever stored (`storage.get_all_posts()`, any verdict) through the current code end to end — content pre-filters (`_replay_text_prefilters()` in `core/evaluate.py`, kept in sync with `_scan_group_page`'s (`fb_scraper.py`) equivalent block), a real LLM call, `_evaluate_post_data`, and the distance check — and prints only the posts whose verdict would now differ from what's stored. No sheet writes, no DB writes (so it can't clobber a hand-edited sheet row or the DB's real cached verdict with a test run's result), no browser, so no checkpoint risk — but it does re-spend one LLM call per stored post. To actually commit anything `--replay` surfaces as newly-matching, re-run the real scraper with `--live`. Known limitation: it doesn't re-apply the `MAX_POST_AGE_DAYS` cutoff, since the stored post date is a frozen relative string (e.g. "3 days ago") that goes stale the moment real time passes; `--prune` runs the retention cleanup below standalone, no browser; `--recompute-scores` rewrites every existing sheet row's `ציון התאמה` cell with the current `scoring.compute_fit_score()` formula, reconstructing a fields dict from the row's own cells (`apartment_bot._fields_from_sheet_row()` — lat/lon aren't persisted anywhere so the location score's directional adjustment comes out neutral for these rows, everything else is the real current formula), no browser, no Maps/Gemini calls — needed whenever the scoring formula itself changes, since old rows are otherwise stuck holding whatever score the previous formula gave them; `--backfill-ratings` re-sends every existing sheet row to Telegram with the 0-10 rating keyboard (see Telegram section below), for collecting human judgment on listings added before the rating keyboard existed.
...
- `scoring.compute_fit_score(fields)` produces a 0-100 int, appended as `_build_row()`'s last cell (the sheet's `ציון התאמה` column). Informational only — doesn't affect filtering, dedupe, or `dedupe_and_sort_sheet()`'s sort order (still newest-post-date-first).
- Three weighted components (`SCORE_WEIGHT_PRICE`/`SCORE_WEIGHT_DISTANCE`/`SCORE_WEIGHT_AMENITIES` in config.py, default 40/40/20, should sum to 100): price (cheaper within `[MIN_PRICE, MAX_PRICE]` scores higher), walking distance (closer to 0 scores higher, floor at `MAX_WALKING_DISTANCE_KM`), amenities (elevator + non-empty parking + shelter, each worth an equal share of the amenities weight).
- **Agent-fee adjustment**: if `is_agent` is true, the *scoring* price (not the displayed price) is inflated by `price_val / AGENT_FEE_AMORTIZE_MONTHS` (default 12) — a one-time broker's commission (~one month's rent) amortized over a year of lease, so an agent listing competes against a private one on effective monthly cost rather than sticker price.

## Telegram (optional push + voting)

- Kill switch: `config.TELEGRAM_ENABLED` (default `False`). When `True`, `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` must be set in `.env` — both modules load them via their own `load_dotenv()` call (not coupled to `env.py`'s import order).
- `telegram_notifier.send_listing_alert(post_url, fields, score)` fires once a batch of matches is actually written to the sheet (both `_scan_group_page`'s batched flush and `reparse_rejected_posts()`'s per-row append) — never on a dry run, matching the rule that dry runs commit nothing anywhere. Wrapped in try/except at every call site: a Telegram outage prints a `WARNING` and never breaks the scrape — the match is already safely in the sheet regardless.
- The message carries a `telegram_notifier.build_listing_keyboard(up, down, group_id, post_id)` keyboard: the ⭐/🗑 vote row plus two rows of 0-10 rating buttons, all built off `callback_data` = `"{up|down}:{group_id}:{post_id}"` / `"rate:{score}:{group_id}:{post_id}"` (the FB group+post id pair, reconstructable back into the canonical post URL — see `_canonical_post_url`) rather than the full URL, to stay under Telegram's 64-byte callback_data limit. `build_listing_keyboard()` is the single source for this layout — both the initial `send_listing_alert()` and every `bot_listener.py` tally-refresh reuse it, so a vote tap can't wipe the rating row off the message or vice versa.
- `bot_listener.py` is a **separate long-running process** (`python bot_listener.py`, not invoked by the scraper) that long-polls `getUpdates` and branches on the `callback_data` prefix:
  - `up`/`down` — records the vote via `storage.record_vote()` (upsert — a voter's latest tap wins), edits the button labels to show the running tally (`⭐ מעניין (N)` / `🗑 לא רלוונטי (M)`), and drives the sheet actions below.
  - `rate` — records a 0-10 score via `storage.record_rating()` (same upsert rule) and just acknowledges with an `answerCallbackQuery` toast (`"ניקוד N/10 נשמר"`). No sheet side effect at all — pure data collection, meant as the eventual training set for regressing `scoring.compute_fit_score()`'s weights against real human judgment instead of the current guessed ones. Read back via `storage.get_ratings_with_listing_data()`, which joins `ratings` against `posts` (price/rooms/distance/`parsed_json`) so a rating comes with the listing's actual attributes attached.
  - Resumes from `storage.get_telegram_offset()` after a restart either way, so it never replays old votes or ratings.
- `apartment_bot.py --backfill-ratings` pushes every row currently in the sheet through `send_listing_alert()` again (1.2s pacing between sends, well under Telegram's per-chat rate limit) so members can retroactively rate apartments that were added before the rating keyboard existed — the fastest way to build up a non-trivial training set without waiting for new matches to trickle in. No browser, no Maps/Gemini calls; reuses the same row→fields reconstruction as `--recompute-scores` (see below), so it inherits the same lat/lon gap (neutral direction-score component, doesn't affect the message itself).
- Storage: `posts.telegram_chat_id`/`telegram_message_id` (which message to edit), `votes` table (`url`, `voter_id`, `voter_name`, `vote`, `voted_at`, PK on `url`+`voter_id`), `ratings` table (`url`, `voter_id`, `voter_name`, `score`, `rated_at`, PK on `url`+`voter_id`), `telegram_state` (single-row `update_offset` for the listener's poll cursor).
- `doctor.py --alert` uses `telegram_notifier.send_failure_alert()` to DM a summary when any health check FAILs — the only other caller of `telegram_notifier`.
- **Vote-driven sheet actions** (`bot_listener.py`, on every `callback_query`): reads/writes the sheet directly (its own `_open_sheet()`/gspread client, separate from `sheets.py`'s), finding the listing's row by URL via `sheet.find(url, in_column=1)` — **this returns `None` on no match in the installed gspread version, it does not raise**, so `_find_row()` checks for `None` explicitly before dereferencing; a bare try/except around `.find()` alone would still crash.
  - `config.VOTES_TO_REMOVE` (default 2) down-votes → the row is deleted from the sheet outright (`sheet.delete_rows`); the DB `posts` row/verdict is left untouched, so `_known_added_listing_keys()` still treats it as a known listing and a later crosspost of the same apartment is silently skipped as `duplicate_listing` rather than resurfacing — deliberate, matches "the group already said no."
  - Every up-vote bumps the row's `ציון התאמה` (fit score) cell by `config.SCORE_VOTE_BOOST` (default 10), clamped to 100 (`_boost_score()`) — a direct sheet edit, not a recompute of `scoring.compute_fit_score()`.
  - `config.VOTES_TO_HIGHLIGHT` (default 2) up-votes → `telegram_notifier.send_highlight_alert()` sends a distinct, button-less "🔥" message built from that row's own cells (price/rooms/distance/address), not from `_evaluate_post_data()`'s `fields` dict (bot_listener.py never sees that — it only has the callback's group/post id and whatever's already in the sheet).
  - No column for the vote tally exists in the sheet (removed by design — score bump + row removal are the only sheet-visible effects of a vote); the running ⭐/🗑 count only ever appears on the Telegram message's button labels.
- **Run-start/run-finish alerts** (`run_scraper()`, gated on `live and TELEGRAM_ENABLED` — never on a dry run): `telegram_notifier.send_run_started(total_groups)` fires once `TARGET_URLS` is shuffled, before the login phase; `telegram_notifier.send_run_finished(stats)` fires at the very end (after dedupe/prune), with the same numbers as the console summary line (groups scanned, posts seen, pre-filtered, sent to LLM, added, Maps calls this month, checkpoints hit). Both wrapped in try/except — a Telegram failure here only prints a `WARNING`, never aborts the run.
...
- `backups/` (bot_data.db snapshots from `backup_db.py`) is gitignored too — same DB contents, same reasoning.
