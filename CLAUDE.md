...
| File | Purpose |
|---|---|
| `apartment_bot.py` | CLI entrypoint only: `--stats`/`--reparse-rejected`/`--replay`/`--prune`/`__main__` dispatch to `run_scraper()`. As of the Phase 0-9 extraction (2026-07-26), the scraping/LLM/Sheets logic that used to all live here has moved to the modules below — see each for what it owns. |
...
- The message carries an inline ⭐/🗑 keyboard with `callback_data` = `"{up|down}:{group_id}:{post_id}"` (the FB group+post id pair, reconstructable back into the canonical post URL — see `_canonical_post_url`) rather than the full URL, to stay under Telegram's 64-byte callback_data limit.
- `bot_listener.py` is a **separate long-running process** (`python bot_listener.py`, not invoked by the scraper) that long-polls `getUpdates`, records each vote via `storage.record_vote()` (upsert — a voter's latest tap wins), and edits the original message's button labels to show the running tally (`⭐ מעניין (N)` / `🗑 לא רלוונטי (M)`). Resumes from `storage.get_telegram_offset()` after a restart, so it never replays old votes.
- Storage: `posts.telegram_chat_id`/`telegram_message_id` (which message to edit), `votes` table (`url`, `voter_id`, `voter_name`, `vote`, `voted_at`, PK on `url`+`voter_id`), `telegram_state` (single-row `update_offset` for the listener's poll cursor).
