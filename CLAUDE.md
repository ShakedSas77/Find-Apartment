...
- Kill switch: `config.TELEGRAM_ENABLED` (default `False`). When `True`, `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` must be set in `.env` — both modules load them via their own `load_dotenv()` call (not coupled to `apartment_bot.py`'s import order).
...
- **Vote-driven sheet actions** (`bot_listener.py`, on every `callback_query`): reads/writes the sheet directly (its own `_open_sheet()`/gspread client, separate from `apartment_bot.py`'s), finding the listing's row by URL via `sheet.find(url, in_column=1)` — **this returns `None` on no match in the installed gspread version, it does not raise**, so `_find_row()` checks for `None` explicitly before dereferencing; a bare try/except around `.find()` alone would still crash.
